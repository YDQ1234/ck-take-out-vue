...
const init = async () => {
  ...
  const { data: res } = await getAbleCategoryListAPI({ page: 1, pageSize: 100, type: 2 })
  console.log('分类列表')
...
const onFileChange1 = async (e: Event) => {
  ...
  if (files && files.length > 0) {
    ...
    try {
      ...
      const imageUrl = await uploadImageToOSS(file, 'setmeal')
      form.pic = imageUrl
...
<template>
  ...
  <el-card>
    ...
    <el-form :model="form" :rules="rules" ref="addRef">
     ...
     <el-form-item label="套餐图片" :label-width="formLabelWidth" prop="pic">
                       ...
                       <el-button type="primary" @click="chooseImg" :disabled="isUploading">
