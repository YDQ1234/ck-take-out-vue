...
import { getAbleCategoryListAPI } from '@/api/category'
import { getDishPageListAPI } from '@/api/dish'
import { getAbleDishListAPI } from '@/api/dish'
...
const getAllDishList = async () => {
  const { data: res } = await getAbleDishListAPI({ page: 1, pageSize: 100, type: 1})
  allDishList.value = res.data.records
}

// 拿到左侧菜品分类
const getdishCategoryList = async () => {
  const { data: res } = await getAbleCategoryListAPI({ page: 1, pageSize: 100, type: 1 })
...
const getDishList = async (id: number) => {
  const { data: res } = await getAbleDishListAPI({ page: 1, pageSize: 100, categoryId: id })
...
const getDishForName = async (name: string) => {
  const { data: res } = await getAbleDishListAPI({ page: 1, pageSize: 100, name })
  let newArr = res.data.records
  // 加个dishId名，和后端名称对应！
  newArr.forEach((n: any) => {
    n.dishId = n.id
